...
(async function() {
  ...
  function subscribeRoster() {
    db.collection(ROSTER_COL).onSnapshot(snap => {
      ...
      snap.forEach(doc => {
        const d = doc.data();
        if (grouped[d.year]) grouped[d.year].push({ id: doc.id, ...d });
      });
      rosterData = grouped;
  ...
  function getRosterForYear(year) {
    return rosterMigrated ? rosterData[year] : ROSTER_SEED_DATA[year];
  }

  // One-time: copies the seed roster data into Firestore, enabling live
  // editing. Safe to run once; each student becomes its own document
  // (id: "{year}_{ucid}") so individual group swaps are simple, targeted
  // writes rather than read-modify-write on one large document.
  async function migrateRosterToFirestore() {
    if (rosterMigrated) { showToast('Roster already migrated to Firestore — editing is live'); return; }
    const allStudents = [];
    Object.keys(ROSTER_SEED_DATA).forEach(year => {
      ROSTER_SEED_DATA[year].forEach(s => allStudents.push({ year, ...s }));
    });
    if (!confirm(`This will migrate ${allStudents.length} student roster entries into Firestore, enabling live editing. Proceed?`)) return;
    ...
    for (let i = 0; i < allStudents.length; i += BATCH_SIZE) {
      ...
      chunk.forEach(s => {
        const { ucid, year, ...groups } = s;
        batch.set(db.collection(ROSTER_COL).doc(`${year}_${ucid}`), { year, ucid, ...groups });
      });
      try { await batch.commit(); done += chunk.length; } catch (err) { console.error('[Roster migration error]', err); }
    }
    showToast(`Migrated ${done} roster entries — editing is now live`);
  }

  // Updates a single student's group value for one scheme (e.g. group6,
  // group5, group4, group3, group2) and persists it to Firestore.
  async function updateRosterGroup(docId, field, newValue) {
    try {
      await db.collection(ROSTER_COL).doc(docId).set({ [field]: newValue }, { merge: true });
  ...
  function openGroupRoster(rosterYear) {
    ...
    const editable = isAdmin && rosterMigrated;
    let title, fields; // fields: [{key, label}]
    if (rosterYear === 'year1') {
      title = 'Year 1 Lab Groups';
      fields = [{key:'group6', label:'6-Group Scheme'}, {key:'group5', label:'5-Group Scheme'}];
    } else if (rosterYear === 'year2') {
      title = 'Year 2 Lab Groups';
      fields = [{key:'group4', label:'Group'}];
    } else {
      title = 'Year 3 (505) Lab Groups';
      fields = [{key:'group3', label:'3-Group Scheme'}, {key:'group2', label:'2-Group Scheme'}];
    }
    const headerCols = ['UCID', ...fields.map(f => f.label)];
    const bodyRows = students.map(s => {
      const docId = s.id || `${rosterYear}_${s.ucid}`;
      const cells = fields.map(f => {
        const badge = renderGroupBadge(s[f.key]);
        return editable
          ? `<td class="roster-editable-cell" data-doc-id="${escapeHtml(docId)}" data-field="${f.key}" data-current="${escapeHtml(s[f.key]||'')}" title="Click to change this student's group">${badge}</td>`
          : `<td>${badge}</td>`;
      }).join('');
      return `<tr><td>${escapeHtml(s.ucid)}</td>${cells}</tr>`;
    });

    const migrationNotice = (isAdmin && !rosterMigrated)
      ? `<div style="padding:8px 12px;background:var(--accent-light);border-radius:6px;font-size:11.5px;margin-bottom:10px">
           Roster is currently read-only reference data. <button class="btn btn-secondary" id="migrate-roster-btn" style="padding:2px 10px;font-size:11px;margin-left:4px">Enable Editing</button>
         </div>`
      : '';
    ...
    if (editable) {
      modal.querySelectorAll('.roster-editable-cell').forEach(cell => {
        cell.addEventListener('click', () => {
          ...
          const next = prompt(`New group value (current: "${current || '(none)'}"):`, current);
          if (next === null || next.trim() === current) return;
          updateRosterGroup(cell.dataset.docId, cell.dataset.field, next.trim());
        });
      });
    }
  ...
  // One-time: deletes ONLY the exact (course, date, topic) rows confirmed
  // present in the source spreadsheet for 304/306/315/317/319 — never a
  // blanket "all LAB/SRL for this course" match, specifically to avoid
  // catching any standalone SRL sessions that exist outside the lab
  ...
  async function deleteYear2RemainingCoursesForRebuild() {
    ...
    if (!matches.length) { showToast('No matching rows found — may already be cleared'); return; }
    if (!confirm(`Found ${matches.length} rows across 304/306/315/317/319 matching the exact topics confirmed in the source file (out of 77 expected combinations). Any standalone SRL sessions outside this list are NOT touched. After deleting, import course_304_306_315_317_319_rebuild.csv via Import CSV. Proceed?`)) return;
    const BATCH_SIZE = 150; let deleted = 0;
    for (let i = 0; i < matches.length; i += BATCH_SIZE) {
      const chunk = matches.slice(i, i + BATCH_SIZE);
      const batch = db.batch();
